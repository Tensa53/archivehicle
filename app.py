...
def get_manufacturer_stats(manufacturer):
    ...
    # get all vehicles count per manufacturer and by possible values of the basic properties
    ...
    # get all vehicles count per manufacturer and by possible values of the mechanical_details properties
    ...
    # get all vehicles count per manufacturer and by possible values of the body_details properties
...
def get_mechanical_details(form):
    ...
    if check:
        mechanical_details = {"engine": engine, "cylinders": int(cylinders), "fuel": fuel, "mileage": float(mileage),
...
def get_body_details(form):
    ...
    if check:
        body_details = {"trim": trim, "doors": int(doors), "exterior": exterior, "interior": interior}
        return body_details
    else:
        return None
...
def process_vehicle_form():
    ...
    new_vehicle = {"name": name, "model": model, "chassis": chassis, "type": type, "year": int(year),
                   "price": float(price),
                   "image": image,
                   "description": description,
                   "manufacturer_id": ObjectId(manufacturer)}

    if mechanical_details is not None:
        new_vehicle["mechanical_details"] = mechanical_details

    if body_details is not None:
        new_vehicle["body_details"] = body_details
...
def insert_vehicle():
    ...
    return render_template('insert_vehicle.html',
                           manufacturers=list(manufacturers_col.find().sort('name', pymongo.ASCENDING)),
...
def process_query():
    ...
    if vehicle_price_below != "":
        filter_list['priceBelow'] = vehicle_price_below
    ...
    if vehicle_cylinders != "No Cylinders Filter":
        filter_list['cylinders'] = vehicle_cylinders
    ...
    if vehicle_drivetrain != "No Drivetrain Filter":
        filter_list['drivetrain'] = vehicle_drivetrain
    ...
    if vehicle_doors != "No Doors Filter":
        filter_list['doors'] = vehicle_doors
    ...
    if vehicle_chassis != "No Chassis Filter":
        filter_list['chassis'] = vehicle_chassis
...
def search_a_vehicle():
    ...
    if len(vehicles_found) > 0:
        return render_template('index.html', year=year, cylinders=cylinders, fuel=fuel,
                               ...
                               vehicles=vehicles_found, filters=filters)
    else:
        return render_template('index.html', year=year, cylinders=cylinders, fuel=fuel,
                               drivetrain=drivetrain, doors=doors, chassis=chassis, manufacturer=manufacturer,
                               empty_message="No Vehicles Found", filters=filters)
